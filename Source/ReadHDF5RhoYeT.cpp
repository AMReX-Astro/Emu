...
#ifdef AMREX_USE_MPI
#include <mpi.h>
#define BCAST(buffer, size) \
    MPI_Bcast(buffer, size, MPI_BYTE, my_reader_process, MPI_COMM_WORLD)
#else
#define BCAST(buffer, size) \
    do { /* do nothing */   \
    } while (0)
...
void ReadInputRhoYeT(const std::string hdf5_background_rho_Ye_T) {
    ...
    amrex::Print() << "(ReadHDF5RhoYeT.cpp) Using hdf5: "
                   << hdf5_background_rho_Ye_T << std::endl;
    ...
    if (doIO && !file_is_readable(hdf5_background_rho_Ye_T)) {
        AMREX_ASSERT_WITH_MESSAGE(false,
                                  "Could not read hdf5_background_rho_Ye_T");
    }

    HDF5_ERROR(file = H5Fopen(hdf5_background_rho_Ye_T.c_str(), H5F_ACC_RDONLY,
                              H5P_DEFAULT));
...
#define READ_BCAST_EOSTABLE_HDF5(NAME, OFF, DIMS)                          \
    do {                                                                   \
        READ_BCAST_EOS_HDF5(NAME,                                          \
                            &allbackgroundYeTrhos_temp[(OFF) * (DIMS)[1]], \
                            H5T_NATIVE_DOUBLE, H5S_ALL, (DIMS)[1]);        \
    } while (0)
    ...
    READ_BCAST_EOS_HDF5("xmin", &xmin_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    READ_BCAST_EOS_HDF5("ymin", &ymin_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    READ_BCAST_EOS_HDF5("zmin", &zmin_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    READ_BCAST_EOS_HDF5("xmax", &xmax_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    READ_BCAST_EOS_HDF5("ymax", &ymax_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    READ_BCAST_EOS_HDF5("zmax", &zmax_, H5T_NATIVE_DOUBLE, H5S_ALL, 1);
    ...
    if (!(allbackgroundYeTrhos_temp =
              myManagedArena.allocate(ncellx_ * ncelly_ * ncellz_ * 6))) {
        printf("(ReadEosTable.cpp) Cannot allocate memory for EOS table");
        assert(0);
    }
    // Allocate memory for tables
    if (!(rho_array_input =
              myManagedArena.allocate(ncellx_ * ncelly_ * ncellz_))) {
        printf("(ReadEosTable.cpp) Cannot allocate memory for EOS table");
        assert(0);
    }
    if (!(T_array_input =
    ...
    for (int i = 0; i < ncellx_ * ncelly_ * ncellz_; i++) {
        rho_array_input[i] =
            allbackgroundYeTrhos_temp[i + 0 * ncellx_ * ncelly_ * ncellz_];
        T_array_input[i] =
            allbackgroundYeTrhos_temp[i + 1 * ncellx_ * ncelly_ * ncellz_];
        Ye_array_input[i] =
            allbackgroundYeTrhos_temp[i + 2 * ncellx_ * ncelly_ * ncellz_];
        vupx_array_input[i] =
            allbackgroundYeTrhos_temp[i + 3 * ncellx_ * ncelly_ * ncellz_];
        vupy_array_input[i] =
            allbackgroundYeTrhos_temp[i + 4 * ncellx_ * ncelly_ * ncellz_];
        vupz_array_input[i] =
            allbackgroundYeTrhos_temp[i + 5 * ncellx_ * ncelly_ * ncellz_];
    }

    // free memory of temporary array
    myManagedArena.deallocate(allbackgroundYeTrhos_temp,
                              ncellx_ * ncelly_ * ncellz_ * 6);
