...
void ReadNuLibTable(const std::string nulib_table_name) {
    ...
    amrex::Print() << "(ReadNuLibTable.cpp) Using table: " << nulib_table_name
                   << std::endl;
    ...
    HDF5_ERROR(
        file = H5Fopen(nulib_table_name.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT));
...
#define READ_BCAST_EOSTABLE_HDF5(NAME, OFF, DIMS)                     \
    do {                                                              \
        READ_BCAST_EOS_HDF5(NAME, &alltables_temp[(OFF) * (DIMS)[1]], \
    ...
    READ_BCAST_EOS_HDF5("number_species", &nspecies_, H5T_NATIVE_INT, H5S_ALL,
                        1);
    ...
    if (!(alltables_temp = myManagedArena.allocate(
              nrho_ * ntemp_ * nye_ * nspecies_ * ngroup_ * NTABLES_NULIB))) {
    ...
    hsize_t table_dims[2] = {
        NTABLES_NULIB, (hsize_t)nrho_ * ntemp_ * nye_ * nspecies_ * ngroup_};
    ...
    //This is not log yet.
    READ_BCAST_EOS_HDF5("rho_points", logrho_nulib, H5T_NATIVE_DOUBLE, H5S_ALL,
                        nrho_);
    READ_BCAST_EOS_HDF5("temp_points", logtemp_nulib, H5T_NATIVE_DOUBLE,
                        H5S_ALL, ntemp_);
    READ_BCAST_EOS_HDF5("ye_points", yes_nulib, H5T_NATIVE_DOUBLE, H5S_ALL,
                        nye_);
    READ_BCAST_EOS_HDF5("neutrino_energies", group_nulib, H5T_NATIVE_DOUBLE,
                        H5S_ALL, ngroup_);

    READ_BCAST_EOS_HDF5("bin_bottom", energy_bottom, H5T_NATIVE_DOUBLE, H5S_ALL,
                        ngroup_);
    READ_BCAST_EOS_HDF5("bin_top", energy_top, H5T_NATIVE_DOUBLE, H5S_ALL,
                        ngroup_);
    ...
    if (!(alltables_nulib = myManagedArena.allocate(
              nrho_ * ntemp_ * nye_ * nspecies_ * ngroup_ * NTABLES_NULIB))) {
        printf("(ReadNuLibTable.cpp) Cannot allocate memory for NuLib table");
        AMREX_ASSERT(0);
    }
    ...
    // set up some vars
    NULIBVAR(dtemp) =
        (logtemp_nulib[ntemp_ - 1] - logtemp_nulib[0]) / (1.0 * (ntemp_ - 1));
    ...
    NULIBVAR(drho) =
        (logrho_nulib[nrho_ - 1] - logrho_nulib[0]) / (1.0 * (nrho_ - 1));
    ...
    NULIBVAR(drholintempyei) =
        NULIBVAR(drhoi) * NULIBVAR(dlintempi) * NULIBVAR(dyei);
