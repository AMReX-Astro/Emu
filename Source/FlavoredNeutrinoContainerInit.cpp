...
void FlavoredNeutrinoContainer::CreateParticlesAtBoundary(
    const TestParams* parms, const Real current_dt) {
...
#pragma omp parallel
    ...
    for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi) {
        ...
        Gpu::ManagedVector<unsigned int> counts(
            tile_box.numPts(),
            0);  //PODVector<int, ManagedArenaAllocator<int> >  counts(n, 0)
        ...
        amrex::ParallelFor(tile_box, [=] AMREX_GPU_DEVICE(int i, int j,
                                                          int k) noexcept {
            for (int i_part = 0; i_part < nlocs_per_cell; i_part++) {
                ...
                switch (DIRECTION) {
                    ...
                    case BoundaryParticleCreationDirection::I_MINUS:
                        if (i == ncellx - 1 - buffer)
                            create_particle_this_cell = true;
                        break;
                ...
                int ny = hi.y - lo.y + 1;
                int nz = hi.z - lo.z + 1;
                unsigned int uix = amrex::min(
                    nx - 1,
                    amrex::max(
                        0,
                        ix));  //Forces the value of 'uix' to be in the range of 0 to nx-1.
                unsigned int uiy = amrex::min(ny - 1, amrex::max(0, iy));
                unsigned int uiz = amrex::min(nz - 1, amrex::max(0, iz));
                unsigned int cellid = (uix * ny + uiy) * nz + uiz;
                pcount[cellid] += ndirs_per_loc;
        ...
        Gpu::exclusive_scan(
            ...
            offsets.begin());  //This sets the value of "offsets"
                ...
                for (int i_loc = 0; i_loc < nlocs_per_cell; i_loc++) {
                    ...
                    switch (DIRECTION) {
                        ...
                        case BoundaryParticleCreationDirection::I_MINUS:
                            if (i == ncellx - 1 - buffer)
                                create_particle_this_cell = true;
                            x = plo[0] + (ncellx - buffer) *
                                             dx[0];  //VC, upper x boundary
                            y = plo[1] + (j + r[1]) * dx[1];  //CC
                            z = plo[2] + (k + r[2]) * dx[2];  //CC
                            break;
                        ...
                        case BoundaryParticleCreationDirection::J_MINUS:
                            if (j == ncelly - 1 - buffer)
                                create_particle_this_cell = true;
                            y = plo[1] + (ncelly - buffer) *
                                             dx[1];  //VC, upper y boundary
                            x = plo[0] + (i + r[0]) * dx[0];  //CC
                            z = plo[2] + (k + r[2]) * dx[2];  //CC
                            break;
                        ...
                        case BoundaryParticleCreationDirection::K_MINUS:
                            if (k == ncellz - 1 - buffer)
                                create_particle_this_cell = true;
                            z = plo[2] + (ncellz - buffer) *
                                             dx[2];  //VC, upper z boundary
                            x = plo[0] + (i + r[0]) * dx[0];  //CC
                            y = plo[1] + (j + r[1]) * dx[1];  //CC
                            break;
                    ...
                    for (int i_direction = 0; i_direction < ndirs_per_loc;
                        ...
                        if (parms->IMFP_method == 1) {
                            const Real V_momentum =
                                4 * MathConst::pi *
                                (pow(p.rdata(PIdx::pupt) + parms->delta_E / 2,
                                     3) -
                                 pow(p.rdata(PIdx::pupt) - parms->delta_E / 2,
                                     3)) /
                                (3 * ndirs_per_loc * parms->nppc[0] *
                                 parms->nppc[1] * parms->nppc[2]);
                            ...
                            switch (DIRECTION) {
                                ...
                                case BoundaryParticleCreationDirection::I_PLUS:
                                    p.rdata(PIdx::Vphase) =
                                        dx[1] * dx[2] * clight * dt *
                                        V_momentum * std::abs(pupx_ / pupt_);
                                    break;
                                ...
                                case BoundaryParticleCreationDirection::J_PLUS:
                                    p.rdata(PIdx::Vphase) =
                                        dx[0] * dx[2] * clight * dt *
                                        V_momentum * std::abs(pupy_ / pupt_);
                                    break;
                                ...
                                case BoundaryParticleCreationDirection::K_PLUS:
                                    p.rdata(PIdx::Vphase) =
                                        dx[0] * dx[1] * clight * dt *
                                        V_momentum * std::abs(pupz_ / pupt_);
                                    break;
