...
void ReadEosTable(const std::string nuceos_table_name) {
    ...
    amrex::Print() << "(ReadEosTable.cpp) Using table: " << nuceos_table_name
                   << std::endl;
    ...
    HDF5_ERROR(
        file = H5Fopen(nuceos_table_name.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT));
...
#define READ_BCAST_EOSTABLE_HDF5(NAME, OFF, DIMS)                     \
    ...
    if (!(alltables_temp =
              myManagedArena.allocate(nrho_ * ntemp_ * nye_ * NTABLES))) {
    ...
    READ_BCAST_EOS_HDF5("energy_shift", &energy_shift_, H5T_NATIVE_DOUBLE,
                        H5S_ALL, 1);
    ...
    for (int i = 0; i < nrho_; i++) {
        ...
        logrho[i] =
            logrho[i] *
            log(10.);  //Let's not convert units yet. Only convert log_10(rho) to ln(rho).
    }
    ...
    for (int i = 0; i < nrho_ * ntemp_ * nye_; i++) {
        {  // pressure
            int idx = 0 + NTABLES * i;
            alltables[idx] =
                alltables[idx] * log(10.0);  //Let's not convert units yet.
        }
