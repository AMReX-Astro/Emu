...
void evolve_flavor(const TestParams* parms)
{
	...
	bool write_plot_particles = parms->write_plot_particles_every > 0 && (step+1) % parms->write_plot_particles_every == 0;
        ...
        const Real dt = compute_dt(geom,parms->cfl_factor,state,neutrinos,parms->flavor_cfl_factor,parms->max_adaptive_speedup, parms);
    ...
    const Real starting_dt = compute_dt(geom,parms->cfl_factor, state,neutrinos_old,parms->flavor_cfl_factor, parms->max_adaptive_speedup, parms);
