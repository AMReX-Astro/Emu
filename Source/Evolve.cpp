...
Real compute_dt(const Geometry& geom, const MultiFab& state,
    ...
    AMREX_ASSERT(parms->cfl_factor > 0.0 || parms->flavor_cfl_factor > 0.0 ||
    ...
    if (parms->cfl_factor > 0.0) {
        dt_translation = std::min(std::min(dxi[0], dxi[1]), dxi[2]) /
                         PhysConst::c * parms->cfl_factor;
    }
...
#include "generated_files/Evolve.cpp_compute_dt_fill"
        ...
        Real Vmax_adaptive =
            amrex::get<0>(rv) + FlavoredNeutrinoContainer::Vvac_max;
        Real Vmax_stupid =
            amrex::get<1>(rv) + FlavoredNeutrinoContainer::Vvac_max;
        ...
        Real dt_flavor_absorption =
            std::numeric_limits<Real>::max();  // Initialize with infinity

        if (parms->attenuation_hamiltonians != 0) {
            dt_flavor_adaptive = PhysConst::hbar / Vmax_adaptive *
                                 parms->flavor_cfl_factor /
                                 parms->attenuation_hamiltonians;
            dt_flavor_stupid = PhysConst::hbar / Vmax_stupid *
                               parms->flavor_cfl_factor /
                               parms->attenuation_hamiltonians;
        }
    ...
    if (dt_translation != 0.0 && dt_flavor != 0.0) {
        ...
        amrex::Error(
            "Timestep selection failed, try using both cfl_factor and "
            "flavor_cfl_factor");
    }
...
void deposit_to_mesh(const FlavoredNeutrinoContainer& neutrinos,
    ...
    amrex::ParticleToMesh(
        ...
        const ParticleInterpolator<SHAPE_FACTOR_ORDER> sx(delta_x,
                                                          shape_factor_order_x);
        const ParticleInterpolator<SHAPE_FACTOR_ORDER> sy(delta_y,
                                                          shape_factor_order_y);
        const ParticleInterpolator<SHAPE_FACTOR_ORDER> sz(delta_z,
                                                          shape_factor_order_z);
        ...
        const int ncomp =
            PIdx::N00_Rebar -
            PIdx::N00_Re;  // real/imaginary components per NxN Hermitian block
        ...
        for (int k = sz.first(); k <= sz.last(); ++k) {
            for (int j = sy.first(); j <= sy.last(); ++j) {
                for (int i = sx.first(); i <= sx.last(); ++i) {
                    ...
                    // every grid moment block,
                    // for neutrinos (nunubar=0) and antineutrinos (nunubar=1).
                        ...
                        for (int m = 0; m < nmoments; ++m) {
                            ...
                            for (int comp = 0; comp < ncomp; ++comp) {
                                ...
                                amrex::Gpu::Atomic::AddNoRet(
                                    ...
                                    vol * p.rdata(particle_component_index) *
                                        moment_factor[m]);
                            }
                        }
                    }
...
#include "generated_files/Evolve.cpp_Vvac_fill"
            ...
            // The following variables contains values interpolated
            // from grid quantities to particle positions
            Real T_pp = 0;  // erg
            Real Ye_pp = 0;
            Real rho_pp = 0;  // g/ccm
...
#include "generated_files/Evolve.cpp_interpolate_from_mesh_fill"
                ...
                // The following variables contains variables interpolated
                // from grid quantities to particle positions
                Real T_pp = 0;  // erg
                Real Ye_pp = 0;
                Real rho_pp = 0;  // g/ccm
...
#include "generated_files/Evolve.cpp_interpolate_from_mesh_fill"
                ...
                // Neutrino inverse mean free path matrix for nucleon absortion:
                // diag( k_e , k_u , k_t )
                Real IMFP_abs[NUM_FLAVORS][NUM_FLAVORS];
                // Antineutrino inverse mean free path matrix for nucleon absortion:
                // diag( kbar_e , kbar_u , kbar_t )
                Real IMFP_absbar[NUM_FLAVORS][NUM_FLAVORS];
                // Neutrino inverse mean free path matrix for scatteting:
                // diag( k_e , k_u , k_t )
                Real IMFP_scat[NUM_FLAVORS][NUM_FLAVORS];
                // Antineutrino inverse mean free path matrix for scatteting:
                // diag( kbar_e , kbar_u , kbar_t )
                Real IMFP_scatbar[NUM_FLAVORS][NUM_FLAVORS];
                // Neutrino equilibrium Fermi-dirac distribution matrix:
                // f_eq = diag( f_e , f_u , f_t )
                Real f_eq[NUM_FLAVORS][NUM_FLAVORS];
                // Antineutrino equilibrium Fermi-dirac distribution matrix:
                // f_eq = diag( fbar_e , fbar_u , fbar_t )
                Real f_eqbar[NUM_FLAVORS][NUM_FLAVORS];
                // Neutrino chemical potential matrix:
                // munu = diag ( munu_e , munu_x)
                Real munu[NUM_FLAVORS][NUM_FLAVORS];
                // Antineutrino chemical potential matrix:
                // munu = diag ( munubar_e , munubar_x)
                Real munubar[NUM_FLAVORS][NUM_FLAVORS];
                ...
                // If opacity_method is 1, the code will use the inverse mean free paths
                // in the input parameters to compute the collision term.
                if (parms->IMFP_method == 0) {
                    // do nothing
                } else if (parms->IMFP_method == 1) {
                    // Read from input parameters file
                    for (int i = 0; i < NUM_FLAVORS; ++i) {
                        // 1/cm : absorption inverse mean free path
                        IMFP_abs[i][i] = parms->IMFP_abs[0][i];
                        // 1/cm : Read absorption inverse mean free path
                        IMFP_absbar[i][i] = parms->IMFP_abs[1][i];
                        // ergs : Read neutrino chemical potential
                        munu[i][i] = parms->munu[0][i];
                        // ergs : Read antineutrino chemical potential
                        munubar[i][i] = parms->munu[1][i];
                    }
                }
                // If opacity_method is 2, the code interpolate inverse mean free paths
                // from NuLib table and electron neutrino chemical potential from EoS table
                // to compute the collision term.
...
//#define DEBUG_INTERPOLATION_TABLES
                    ...
                    amrex::Print()
                        << "(Evolve.cpp) mu_e interpolated = " << mue_out
                        << std::endl;
                    amrex::Print()
                    ...
                    //abort if energy bin cannot be found.
                    if (idx_group == -1) AMREX_ASSERT(0);
                    ...
                    NuLib_tabulated_obj.get_opacities(
                        ...
                        scattering_opacity, keyerr, anyerr, idx_species,
                        idx_group);
                    ...
                    // heavier ones: muon neutrino[0,1], muon antineutruino[1,1]
                    // tau neutrino[0,2], tau antineutrino[1,2]
                    idx_species = 2;
                    NuLib_tabulated_obj.get_opacities(
                        rho, temperature, Ye, absorption_opacity,
                        scattering_opacity, keyerr, anyerr, idx_species,
                ...
                // include Pauli blocking term if requested
                ...
                // Compute the time derivative of \( N_{ab} \)
                // using the Quantum Kinetic Equations (QKE).
                ...
                amrex::ParallelFor(np, [=] AMREX_GPU_DEVICE(int i) {
                    ...
                    if (parms->do_blackhole == 1) {
                        ...
                        double particle_distance_from_bh_center = sqrt(
                            amrex::Math::powi<2>(p.rdata(PIdx::x) -
                                                 parms->bh_center_x) +
                        ...
                        if (particle_distance_from_bh_center <
                            parms->bh_radius) {
                            for (int comp = PIdx::N00_Re; comp < PIdx::TrHN;
                                 ++comp)
                                p.rdata(comp) = 0.0;
                            return;
                        }
                    ...
                    if (p.rdata(PIdx::x) < parms->Lx / parms->ncell[0] ||
                        p.rdata(PIdx::x) >
                            parms->Lx - parms->Lx / parms->ncell[0] ||
                        p.rdata(PIdx::y) < parms->Ly / parms->ncell[1] ||
                        p.rdata(PIdx::y) >
                            parms->Ly - parms->Ly / parms->ncell[1] ||
                        p.rdata(PIdx::z) < parms->Lz / parms->ncell[2] ||
                        p.rdata(PIdx::z) >
                            parms->Lz - parms->Lz / parms->ncell[2]) {
                        for (int comp = PIdx::N00_Re; comp < PIdx::TrHN; ++comp)
                            p.rdata(comp) = 0.0;
                        return;
                    }
