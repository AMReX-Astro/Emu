...
template <typename T>
void append_0D(HighFive::File& file0D, const std::string& datasetname,
               const T value) {
...
void DataReducer::InitializeFiles() {
    if (ParallelDescriptor::IOProcessor()) {
        ...
        file0D.createDataSet("time(s)", dataspace,
                             create_datatype<amrex::Real>(), props);
        file0D.createDataSet("Ntot(1|ccm)", dataspace,
                             create_datatype<amrex::Real>(), props);
        file0D.createDataSet("Ndiff(1|ccm)", dataspace,
                             create_datatype<amrex::Real>(), props);
        for (int i = 0; i < NUM_FLAVORS; i++) {
            file0D.createDataSet(std::string("N") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(
                std::string("N") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(std::string("Fx") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Fy") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Fz") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(
                std::string("Fx") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Fy") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Fz") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
#if NUM_MOMENTS == 3
            file0D.createDataSet(std::string("Pxx") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Pxy") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Pxz") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Pyy") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Pyz") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(std::string("Pzz") + std::to_string(i) +
                                     std::to_string(i) + std::string("(1|ccm)"),
                                 dataspace, create_datatype<amrex::Real>(),
                                 props);
            file0D.createDataSet(
                std::string("Pxx") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Pxy") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Pxz") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Pyy") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Pyz") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
            file0D.createDataSet(
                std::string("Pzz") + std::to_string(i) + std::to_string(i) +
                    std::string("bar(1|ccm)"),
                dataspace, create_datatype<amrex::Real>(), props);
#endif
        }
        file0D.createDataSet("N_offdiag_mag(1|ccm)", dataspace,
                             create_datatype<amrex::Real>(), props);
        file0D.createDataSet("sumTrN", dataspace,
                             create_datatype<amrex::Real>(), props);
        file0D.createDataSet("sumTrHN", dataspace,
                             create_datatype<amrex::Real>(), props);
...
#if NUM_MOMENTS == 3
            j++;
            outfile << j << ":Pxx" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pxy" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pxz" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pyy" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pyz" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pzz" << i << i << "(1|ccm)\t";
            j++;
            outfile << j << ":Pxx" << i << i << "bar(1|ccm)\t";
            j++;
            outfile << j << ":Pxy" << i << i << "bar(1|ccm)\t";
            j++;
            outfile << j << ":Pxz" << i << i << "bar(1|ccm)\t";
            j++;
            outfile << j << ":Pyy" << i << i << "bar(1|ccm)\t";
            j++;
            outfile << j << ":Pyz" << i << i << "bar(1|ccm)\t";
            j++;
            outfile << j << ":Pzz" << i << i << "bar(1|ccm)\t";
#endif
        }
        j++;
...
void DataReducer::WriteReducedData0D(const amrex::Geometry& geom,
    ...
    amrex::Print() << "step=" << step << ", time=" << time << ", dt=" << dt
                   ...
                   << ", TrHN=" << TrHN << ", Vphase=" << Vphase << std::endl;
...
// use the ParReduce function to define reduction operator
    ...
    GpuTuple<
        ...
        Real, ArithmeticArray<Real, NUM_FLAVORS>,
        ArithmeticArray<Real, NUM_FLAVORS>, ArithmeticArray<Real, NUM_FLAVORS>,
        ArithmeticArray<Real, NUM_FLAVORS>, ArithmeticArray<Real, NUM_FLAVORS>,
...
#error "NUM_MOMENTS must be 2 or 3"
                ...
                for (int fi = 0; fi < NUM_FLAVORS; ++fi) {
                    ...
                    for (int fj = fi + 1; fj < NUM_FLAVORS; ++fj) {
                        const Real re =
                            a(i, j, k,
                              GIdx::N00_Re + PIdx::offset(fi, fj, PIdx::Re));
                        const Real im =
                            a(i, j, k,
                              GIdx::N00_Re + PIdx::offset(fi, fj, PIdx::Im));
                        const Real rebar =
                            a(i, j, k,
                              GIdx::N00_Rebar + PIdx::offset(fi, fj, PIdx::Re));
                        const Real imbar =
                            a(i, j, k,
                              GIdx::N00_Rebar + PIdx::offset(fi, fj, PIdx::Im));
                        N_offdiag_mag2 +=
                            re * re + im * im + rebar * rebar + imbar * imbar;
                    }
                }

#if NUM_MOMENTS == 2
                return {Ndiag,  Ndiagbar,  N_offdiag_mag2, Fxdiag,   Fydiag,
                        Fzdiag, Fxdiagbar, Fydiagbar,      Fzdiagbar};
#elif NUM_MOMENTS == 3
                return {Ndiag,      Ndiagbar,   N_offdiag_mag2, Fxdiag,
                        Fydiag,     Fzdiag,     Fxdiagbar,      Fydiagbar,
                        Fzdiagbar,  Pxxdiag,    Pxydiag,        Pxzdiag,
                        Pyydiag,    Pyzdiag,    Pzzdiag,        Pxxdiagbar,
                        Pxydiagbar, Pxzdiagbar, Pyydiagbar,     Pyzdiagbar,
                        Pzzdiagbar};
#else
#error "NUM_MOMENTS must be 2 or 3"
#endif
    ...
    for (int i = 0; i < NUM_FLAVORS; i++) {
        ParallelDescriptor::ReduceRealSum(
            N[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Nbar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fx[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fy[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fz[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fxbar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fybar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Fzbar[i], ParallelDescriptor::IOProcessorNumber());
#if NUM_MOMENTS == 3
        ParallelDescriptor::ReduceRealSum(
            Pxx[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pxy[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pxz[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pyy[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pyz[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pzz[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pxxbar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pxybar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pxzbar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pyybar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pyzbar[i], ParallelDescriptor::IOProcessorNumber());
        ParallelDescriptor::ReduceRealSum(
            Pzzbar[i], ParallelDescriptor::IOProcessorNumber());
#endif
    }
    ParallelDescriptor::ReduceRealSum(N_offdiag_mag2,
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        for (int i = 0; i < NUM_FLAVORS; i++) {
            ...
            append_0D(file0D,
                      ...
                      Nbar[i]);
            append_0D(file0D,
                      std::string("Fx") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Fx[i]);
            append_0D(file0D,
                      std::string("Fy") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Fy[i]);
            append_0D(file0D,
                      std::string("Fz") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Fz[i]);
            append_0D(file0D,
                      std::string("Fx") + std::to_string(i) +
                          std::to_string(i) + std::string("bar(1|ccm)"),
                      Fxbar[i]);
            append_0D(file0D,
                      std::string("Fy") + std::to_string(i) +
                          std::to_string(i) + std::string("bar(1|ccm)"),
...
#if NUM_MOMENTS == 3
            append_0D(file0D,
                      std::string("Pxx") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pxx[i]);
            append_0D(file0D,
                      std::string("Pxy") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pxy[i]);
            append_0D(file0D,
                      std::string("Pxz") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pxz[i]);
            append_0D(file0D,
                      std::string("Pyy") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pyy[i]);
            append_0D(file0D,
                      std::string("Pyz") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pyz[i]);
            append_0D(file0D,
                      std::string("Pzz") + std::to_string(i) +
                          std::to_string(i) + std::string("(1|ccm)"),
                      Pzz[i]);
            append_0D(file0D,
                      std::string("Pxx") + std::to_string(i) +
