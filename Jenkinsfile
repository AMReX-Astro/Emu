pipeline {
    ...
    agent { dockerfile {args '--gpus all -v /mnt/scratch/EOS:/EOS:ro -v /mnt/scratch/NuLib:/NuLib:ro'}} // Use the Dockerfile defined in the root Flash-X directory
    environment {
    ...
    stages {
		...
		stage('Fermi-Dirac Reflecting 1D Spherical'){ steps{
				dir('Exec'){
					sh 'python ../Scripts/initial_conditions/st9_empty_particles_multi_energy.py'
					sh 'python ../Scripts/initial_conditions/nsm_constant_background_rho_Ye_T_writer_spherical.py'
    ...
    post {
        always {
			cleanWs(
				...
				notFailBuild: true,
				patterns: [[pattern: 'submodules', type: 'EXCLUDE']] 
			) // allow submodules to be cached
		} 	
    }
