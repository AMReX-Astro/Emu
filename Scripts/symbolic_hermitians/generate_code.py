...
parser = argparse.ArgumentParser(
    description="Generates code for calculating C = i * [A,B] for symbolic NxN Hermitian matrices A, B, C, using real-valued Real and Imaginary components."
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Template output file to fill in at the location of the string '<>code<>'.",
)
parser.add_argument(
    "-eh", "--emu_home", type=str, default=".", help="Path to Emu home directory."
)
parser.add_argument(
    "-c",
    "--clean",
    action="store_true",
    help="Clean up any previously generated files.",
)
parser.add_argument(
    "-rn",
    "--rhs_normalize",
    action="store_true",
    help="Normalize F when applying the RHS update F += dt * dFdt (limits to 2nd order in time).",
)
    ...
    if template:
        ...
        for l in ft:
            ...
            if loc != -1:
                ...
                indent = (
                    " " * loc
                )  # indent the generated code the same amount as <>code<>
            else:
                if found_code_loc:
                    footer.append(l)
                else:
...
if __name__ == "__main__":
    ...
    write_code(
        code,
        os.path.join(
            args.emu_home, "Source/generated_files", "FlavoredNeutrinoContainer.H_fill"
        ),
    )
    ...
    for t in tails:
        ...
        for v in vars:
            ...
            code += A.header()
    code_string = (
        'attribute_names = {"time", "x", "y", "z", "pupx", "pupy", "pupz", "pupt", '
    )
    ...
    code = [code[i] + "," for i in range(len(code))]
    write_code(
        code, os.path.join(args.emu_home, "Source/generated_files", "Evolve.H_fill")
    )
    ...
    write_code(
        code,
        os.path.join(
            args.emu_home, "Source/generated_files", "Evolve.cpp_grid_names_fill"
        ),
    )
    ...
    for t in tails:
        ...
        for ivar in range(len(deposit_vars)):
            deplist = HermitianMatrix(
                args.N, deposit_vars[ivar] + "{}{}_{}" + t
            ).header()
    ...
    write_code(
        code,
        os.path.join(
            args.emu_home, "Source/generated_files", "Evolve.cpp_deposit_to_mesh_fill"
        ),
    )
    ...
    write_code(
        code, os.path.join(args.emu_home, "Source/generated_files", "Evolve.H_M2_fill")
    )
    ...
    code.append(
        "Vvac_max = "
        + sympy.cxxcode(sympy.simplify(M2length))
        + "*PhysConst::c4/pupt_min;"
    )
    write_code(
        code,
        os.path.join(
    ...
    write_code(
        code,
        os.path.join(args.emu_home, "Source/generated_files", "Evolve.cpp_Vvac_fill"),
    )
    ...
    for component in ["x", "y", "z"]:
        ...
        Fbar = HermitianMatrix(
            args.N, "fab(i\,j\,k\,GIdx::F" + component + "{}{}_{}bar)"
        )
        HSI = F - Fbar
        V_adaptive2 = HSI.SU_vector_magnitude2()
        code.append(
            "V_adaptive2 += " + sympy.cxxcode(sympy.simplify(V_adaptive2)) + ";"
        )

    # put in the units
    code.append(
        "V_adaptive = sqrt(V_adaptive2)*" + sympy.cxxcode(sqrt2GF / cell_volume) + ";"
    )
    ...
    code.append(
        "V_stupid = max(V_stupid," + sympy.cxxcode(rho * Ye / mp * cell_volume) + ");"
    )
    code.append(
        "V_stupid *= " + sympy.cxxcode(2.0 * args.N * sqrt2GF / cell_volume) + ";"
    )
    write_code(
        code,
        os.path.join(
            args.emu_home, "Source/generated_files", "Evolve.cpp_compute_dt_fill"
        ),
    )
    ...
    rhoye = (
        string_interp + "rho)*" + string_interp + "Ye)/PhysConst::Mp/inv_cell_volume"
    )
    ...
    write_code(
        code,
        os.path.join(args.emu_home, "Source/generated_files", "Evolve.cpp_dfdt_fill"),
    )
    ...
    for t in tails:
        ...
        code.append(
            'Convert << "Matrix trace (SumP) is not equal to 1, trace error exceeds 100*maxError: " << std::abs(error) << " > " << 100.*parms->maxError;'
        )
        ...
        for fii in fdlist:
            ...
            code.append("std::ostringstream Convert;")
            code.append(
                'Convert << "Diagonal element '
                + fii[14:20]
                + ' is negative, less than -100*maxError: " << '
                + fii
                + ' << " < " << -100.*parms->maxError;'
            )
        ...
        code.append("std::ostringstream Convert;")
        code.append(
            'Convert << "flavor vector length differs from target length by more than 100*maxError: " << std::abs(error) << " > " << 100.*parms->maxError;'
        )
    ...
    for t in tails:
        ...
        code.append(
            ...
            + t
            + ") = "
            + sympy.cxxcode(sympy.simplify(f.SU_vector_magnitude()))
            + ";"
        )
    write_code(
        code,
        os.path.join(
            args.emu_home,
            "Source/generated_files/FlavoredNeutrinoContainerInit.cpp_set_trace_length",
        ),
    )
