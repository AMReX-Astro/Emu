...
if __name__ == "__main__":
    ...
    ax.plot(t[l1:l2], N_avg_mag[:,0,1][l1:l2],  label = f'Im Omega EMU = {b:.2e} s$^{{-1}}$',  linestyle = 'dashed')
    ax.plot(time, N_eu_julien, label = r'$N_{eu}$ Julien script')
    ax.plot(time[p1:p2], N_eu_julien[p1:p2], label = f'Im Omega Julien = {bj:.2e} s$^{{-1}}$',  linestyle = 'dashed')
    ax.plot(time[p1:p2], 1e23*np.exp(ImOmega_Lucas_LSA*time[p1:p2]), label = f'Im Omega Lucas LSA = {ImOmega_Lucas_LSA:.2e} s$^{{-1}}$', linestyle = 'dashed')
