...
parser.add_argument(
    ...
    "--no_assert",
    action="store_true",
    help="If --no_assert is supplied, do not raise assertion errors if the test error > tolerance.",
)
...
theta12 = 33.82 * np.pi / 180.0  # radians
eV = 1.60218e-12  # erg
...
E = dm21c4 * np.sin(2.0 * theta12) / (8.0 * np.pi * hbar * clight)
rhoYe = 4.0 * np.pi * hbar * clight * mp / (np.tan(2.0 * theta12) * np.sqrt(2.0) * GF)
...
if __name__ == "__main__":
    ...
    V = np.sqrt(2.0) * GF * rhoYe / mp

    # Richers(2019) B3
    C = np.cos(2.0 * theta12) + 2.0 * V * E / dm21c4
    Cbar = np.cos(2.0 * theta12) - 2.0 * V * E / dm21c4
    sin2_eff = np.sin(2.0 * theta12) ** 2 / (np.sin(2.0 * theta12) ** 2 + C ** 2)
    sin2_effbar = np.sin(2.0 * theta12) ** 2 / (np.sin(2.0 * theta12) ** 2 + Cbar ** 2)
    dm2_eff = dm21c4 * np.sqrt(np.sin(2.0 * theta12) ** 2 + C ** 2)
    dm2_effbar = dm21c4 * np.sqrt(np.sin(2.0 * theta12) ** 2 + Cbar ** 2)

    # theoretical oscillation probabilities
    def Psurv(dm2, sin2theta, E):
        return 1.0 - np.sin(t * dm2 / (4.0 * E * hbar)) ** 2 * sin2theta
    ...
    fxx_analytic = 1.0 - Psurv(dm2_eff, sin2_eff, E)
